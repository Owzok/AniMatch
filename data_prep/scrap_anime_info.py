...
def get_anime_ids():
    lst_animes=[]
    lst_years = [x for x in range(2000, 2024)]
    lst_seasons = ["winter", "spring", "summer", "fall"]

    for year in lst_years:
        for season in lst_seasons:
            print("Vamos en ", season, year)
            response =requests.get("https://myanimelist.net/anime/season/"+str(year)+"/"+str(season))
            if response.status_code == 200:
                soup = BeautifulSoup(response.text, 'html.parser')
                
                div_elements = soup.find_all('div', class_='seasonal-anime-list')
                for div_anime_list in div_elements:
                    for anime_window in div_anime_list.find_all("div", class_="js-anime-category-producer"):
                            url = anime_window.find("a", class_="link-title").get("href")
                            lst_animes.append(url.split("/")[-2])


    lst_animes = list(df["Number"].unique()[:-1])
    with open("animes_ids.txt", 'w') as file:
        for number in lst_animes:
            file.write(str(number) + '\n')

with open("./animes_ids.txt", "r") as file:
    # Read the content of the file and split it into lines
    lines = file.read().split('\n')
df = pd.DataFrame({'Number': lines})
